...
gw = int(input("Hvilken gameweek er neste?"))
ft= 1
horizon = int(input("Hvor mange gw frem i tid?"))

def xmin_to_prob(xmin, sub_on=0.5, sub_off=0.3):
    start = min(max((xmin - 25 * sub_on) / (90 * (1-sub_off) +
...
def solve_randomized_problem():
    r = solve_multi_period_fpl(team_id=2821496, gw=gw, ft=ft,
                               horizon=horizon, objective='regular', seed=None, randomized=True)
    print(r['picks'])
    print(r['summary'])
